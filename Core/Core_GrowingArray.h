...
public:
	Core_GrowingArray(int aNrOfRecommendedItems, int anItemIncreaseSize, bool aSafemodeFlag = true);
	Core_GrowingArray();
	Core_GrowingArray(const Core_GrowingArray<Type>& aArray);
	~Core_GrowingArray();
	...
	{
		return Find2< Core_StdComparer< Type >, Type >(anItem, aLookFromIndex);
	}
	...
	{
		return Find2< Core_StdComparer2< Type, Compared >, Compared >(anItem, aLookFromIndex);
	}

	// Returns index to anItem if exist in the array. Otherwise returns -1
	inline int ReverseFind(const Type& anItem, const unsigned int aLookFromReverseIndex = 0) const
	{
		return ReverseFind2< Core_StdComparer< Type >, Type >(anItem, aLookFromReverseIndex);
	}
	...
	// Set operations
	inline void Set_Intersection(const Core_GrowingArray<Type>& anArray); // if A={1,2,3}, B={3,5,6} => {3}
	inline void Set_Union(const Core_GrowingArray<Type>& anArray); // if A={1,2,3}, B={2,3,5,6} => {1,2,3,5,6}
	inline void Set_Difference(const Core_GrowingArray<Type>& anArray); // if A={1,2,3}, B={2,3,5,6} => {1,5,6}

	Core_GrowingArray<Type>& operator=(const Core_GrowingArray<Type>& aArray);
	...
	void Sort(COMPARER& aComparer, int aFirstIndex = -1, int aLastIndex = -1, const bool aReverseFlag = false)
	{
		...
		if (aReverseFlag)
			Core_Algorithm::SortReverse(myItemList + aFirstIndex, myItemList + aLastIndex + 1, aComparer);
		else
			Core_Algorithm::Sort(myItemList + aFirstIndex, myItemList + aLastIndex + 1, aComparer);
	}

	// Sorts the array using operator <
	void Sort(int aFirstIndex = -1, int aLastIndex = -1, const bool aReverseFlag = false)
	{
		Sort(Core_Algorithm::StandardComparer<Type>(), aFirstIndex, aLastIndex, aReverseFlag);
	}
	...
	{
		return FindInSortedArray2< Core_StdComparer< Type >, Type >(anItem);
	}
	...
	{
		return Core_Algorithm::BinarySearch(myItemList, myItemList + myUsedNrOfItems, anItem, ComparerProxy<Comparer, Compared>());
	}

	void Swap(typename Core_GrowingArray<Type>& anArray)
	{
...
template <class Type>
Core_GrowingArray<Type>::Core_GrowingArray(int aNrOfRecommendedItems, int anItemIncreaseSize, bool aSafemodeFlag = true)
{
	myItemList = NULL;
	myUsedNrOfItems = 0;
	myMaxNrOfItems = 0;
...
template <class Type>
Core_GrowingArray<Type>::Core_GrowingArray()
{
	myItemList = NULL;
	myUsedNrOfItems = 0;
	myMaxNrOfItems = 0;
...
template <class Type>
inline void Core_GrowingArray<Type>::MoveToIndex(const int anItemNumber, const int aNewItemNumber)
{
#ifdef Core_HEAVY_DEBUG_GROWINGARRAY_BOUNDSCHECK
	assert(anItemNumber >= 0 && anItemNumber < myUsedNrOfItems && "Core_GrowingArray BOUNDS ERROR!");
	assert(aNewItemNumber >= 0 && aNewItemNumber < myUsedNrOfItems && "Core_GrowingArray BOUNDS ERROR!");
#endif
...
template <class Type>
inline void Core_GrowingArray<Type>::MoveToEndCyclic(const int anItemNumber)
{
#ifdef Core_HEAVY_DEBUG_GROWINGARRAY_BOUNDSCHECK
	assert(anItemNumber >= 0 && anItemNumber < myUsedNrOfItems && "Core_GrowingArray BOUNDS ERROR!");
#endif
...
template <class Type>
inline void Core_GrowingArray<Type>::MoveToEndConserveOrder(const int anItemNumber)
{
#ifdef Core_HEAVY_DEBUG_GROWINGARRAY_BOUNDSCHECK
	assert(anItemNumber >= 0 && anItemNumber < myUsedNrOfItems && "Core_GrowingArray BOUNDS ERROR!");
#endif
...
template <class Type>
inline void Core_GrowingArray<Type>::Reverse()
{
	int i1 = 0;
...
template <class Type>
inline void Core_GrowingArray<Type>::Optimize()
{
...
template <class Type>
void Core_GrowingArray<Type>::Reset()
{
	if (myItemList != NULL)
		Delete(myItemList);
	myItemList = 0;
...
template <class Type>
void Core_GrowingArray<Type>::NonDeletingShutdown()
{
	//int i;
...
template <class Type>
bool Core_GrowingArray<Type>::Init(int aNrOfRecommendedItems, int anItemIncreaseSize, bool aSafemodeFlag)
{
...
template <class Type>
bool Core_GrowingArray<Type>::IsInited()
{
	return (myItemList != NULL);
}

template <class Type>
bool Core_GrowingArray<Type>::ReInit(int aNrOfRecommendedItems, int anItemIncreaseSize, bool aSafemodeFlag)
{
	if (myItemList != NULL)
	{
...
template <class Type>
bool Core_GrowingArray<Type>::Add(const Type& anItem)
{
	// Call Init() before using the array
	//assert( myMaxNrOfItems > 0);
	assert(myItemIncreaseSize > 0);
...
template <class Type>
inline bool Core_GrowingArray<Type>::ReplaceUnique(const Type& aItemToReplace, const Type& aReplacingItem) // Replaces the original element with the new unless there is already one of the new then it deletes the original
{
...
template <class Type>
bool Core_GrowingArray<Type>::Add(const Type* someItems, unsigned int aNumberOfItemsToAdd)
{
	assert(someItems);
...
template <class Type>
inline bool Core_GrowingArray<Type>::AddN(const Type& anItem, unsigned int aNumberOfItemsToAdd)
{
	// Call Init() before using the array
	//assert(myMaxNrOfItems > 0);
	assert(myItemIncreaseSize > 0);
...
template <class Type>
void Core_GrowingArray<Type>::AddUnique(const Type& anItem)
{
	unsigned int i;
...
template <class Type>
void Core_GrowingArray<Type>::RemoveAll()
{
	myUsedNrOfItems = 0;
...
template <class Type>
void Core_GrowingArray<Type>::RemoveNAtEnd(int aCount)
{
	myUsedNrOfItems = __max(0, myUsedNrOfItems - aCount);
}

template <class Type>
void Core_GrowingArray<Type>::RemoveFirstN(const unsigned int aNumToRemove)
{
...
template <class Type>
void Core_GrowingArray<Type>::DeleteAll()
{
...
template <class Type>
bool Core_GrowingArray<Type>::RemoveCyclic(const Type& anItem)
{
	int i;
...
template <class Type>
void Core_GrowingArray<Type>::RemoveCyclicAtIndex(int anItemNumber)
{
...
template <class Type>
inline bool Core_GrowingArray<Type>::Remove(const Type& anItem, bool bReverseSearch)
{
	int i;
...
template <class Type>
inline void Core_GrowingArray<Type>::RemoveAtIndex(const int anItemNumber)
{
...
template <class Type>
bool Core_GrowingArray<Type>::DeleteCyclic(const Type& anItem)
{
	int i;
...
template <class Type>
void Core_GrowingArray<Type>::DeleteCyclicAtIndex(const int anItemNumber)
{
 ...
 */
template <class Type>
int Core_GrowingArray<Type>::SetSize(int aSize)
{
	if (aSize == 0)
	{
		Delete(myItemList);
...
template <class Type>
inline void Core_GrowingArray<Type>::PreAllocSize(const int aSize)
{
...
template <class Type>
inline void Core_GrowingArray<Type>::Respace(const int aSize) // resizes and adds elements to fill up
{
	PreAllocSize(aSize);
...
template <class Type>
bool Core_GrowingArray<Type>::InsertItem(const int anIndex, const Type& anItem)
{
	int i;
...
template <class Type>
bool Core_GrowingArray<Type>::RemoveItemConserveOrder(const int anItemNumber)
{
	int i;
...
// Error-prone!
template <class Type>
Core_GrowingArray<Type>& Core_GrowingArray<Type>::operator=(const Core_GrowingArray<Type>& aArray)
{
	if (this == &aArray)
		return *this;
...
// Error-prone!
template <class Type>
Core_GrowingArray<Type>::Core_GrowingArray(const Core_GrowingArray<Type>& aArray)
{
	myMaxNrOfItems = aArray.myMaxNrOfItems;
	myUsedNrOfItems = aArray.myUsedNrOfItems;
	mySafemodeFlag = aArray.mySafemodeFlag;
...
template <class Type>
void Core_GrowingArray<Type>::Set_Intersection(const Core_GrowingArray<Type>& anArray)
{
	if (&anArray == this)
		return;
...
template <class Type>
void Core_GrowingArray<Type>::Set_Union(const Core_GrowingArray<Type>& anArray)
{
	if (&anArray == this)
		return;
...
template <class Type>
void Core_GrowingArray<Type>::Set_Difference(const Core_GrowingArray<Type>& anArray)
{
	if (&anArray == this)
		return;

	Core_GrowingArray<Type> tmp(0, myItemIncreaseSize, true);
	for (int i = 0; i < anArray.Count(); ++i)
	{
